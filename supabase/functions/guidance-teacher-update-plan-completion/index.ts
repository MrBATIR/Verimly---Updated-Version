// Supabase Edge Function: Rehber Öğretmen Plan Tamamlama Durumu Güncelleme
...
serve(async (req) => {
  ...
  try {
    ...
    const { plan_id, institution_id, is_completed, plan_type } = await req.json();
    if (plan_id === undefined || !institution_id || plan_type === undefined || is_completed === undefined) {
      return new Response(JSON.stringify({ error: 'plan_id, institution_id, plan_type ve is_completed gerekli' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    const { data, error } = await supabaseAdmin.from(tableName).update({ is_completed: is_completed }).eq('id', plan_id).select().single();

    if (error) {
      return new Response(JSON.stringify({ error: 'Plan durumu güncellenemedi', details: error.message }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    return new Response(JSON.stringify({ data, error: null, message: 'Plan durumu başarıyla güncellendi.' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error) {
